...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'request_fund_reservations_usage_policy_check') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'usage_cost_reservations_attempt_token_check') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'usage_cost_reservations_attempt_token_fkey') THEN
