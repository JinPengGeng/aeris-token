...
export function validateExecutorRegistry(value) {
  ...
  const executors = value.executors.map((entry, index) => validateExecutorIdentity(entry, `executor ${index}`));
  ...
  const expectedRoutes = ['agent_analysis', 'sync_conflict_resolver', 'sync_conflict_reviewer'];
