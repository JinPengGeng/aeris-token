...
async function runEarlyFence(request, { requestTimeoutMs = 15_000 } = {}) {
  const { document } = read('.github/workflows/automation-policy-gate.yml');
  const source = document.jobs.publish.steps.find((step) => step.id === 'early_fence').with.script;
  const script = source.replace('const requestTimeoutMs = 15_000;', `const requestTimeoutMs = ${requestTimeoutMs};`);
  assert.notEqual(script, source, 'early fence request timeout constant was not found');
  const prior = {
...
test('publish fences any reusable Policy success before checkout, npm, or runtime begin', () => {
  ...
  assert.equal(steps[1].with.script.match(/github\.request/g)?.length, 1);
...
test('early fence hard-aborts a GitHub request that never settles', async () => {
  let signal = null;
  const startedAt = Date.now();
  await assert.rejects(
    () => runEarlyFence((_route, parameters) => {
      signal = parameters.request?.signal ?? null;
      return new Promise(() => {});
    }, { requestTimeoutMs: 25 }),
    /Policy fence GitHub request timed out: GET \/repos\/\{owner\}\/\{repo\}/,
  );
...
test('early fence supersedes a completed same-App success with a new check run', async () => {
  ...
  const outputs = await runEarlyFence(async (route, parameters) => {
    ...
    if (route === 'PATCH /repos/{owner}/{repo}/check-runs/{check_run_id}') throw new Error('completed check must not be reopened');
  ...
  assert.deepEqual(mutations.map(({ route }) => route.split(' ')[0]), ['POST']);
...
test('early fence tolerates completed exact-generation history across pages and creates a replacement', async () => {
  ...
  const calls = [];
  const completed = structuredClone([...state.values()]);
  const outputs = await runEarlyFence(async (route, parameters) => {
    calls.push({ route, parameters });
    if (route === 'GET /repos/{owner}/{repo}') return { data: { id: 123, full_name: 'JinPengGeng/aeris-token', default_branch: 'main' } };
    if (route === 'GET /repos/{owner}/{repo}/git/ref/{ref}') return { data: { ref: 'refs/heads/main', object: { sha: policy } } };
    ...
    if (route === 'GET /repos/{owner}/{repo}/commits/{ref}/check-runs') {
      ...
      if (parameters.page === 2) return { data: { check_runs: [state.get(77)] } };
      throw new Error(`unexpected check-list page ${parameters.page}`);
    }
    if (route === 'POST /repos/{owner}/{repo}/check-runs') {
      const next = { id: 1099, conclusion: null, app: { id: 9001, slug: 'aeris-token-policy' }, ...parameters };
      state.set(1099, next);
      return { data: next };
    }
    if (route === 'GET /repos/{owner}/{repo}/check-runs/{check_run_id}') return { data: state.get(parameters.check_run_id) };
  ...
  assert.equal(calls.filter(({ route }) => route.startsWith('POST ')).length, 1);
  assert.deepEqual([...state.values()].filter(({ id }) => id !== 1099), completed);
});

test('early fence rejects a current generation owned by another App before any mutation', async () => {
  const head = 'a'.repeat(40);
  const base = 'b'.repeat(40);
  const policy = 'c'.repeat(40);
  const external = `aeris-policy:v1:123:37:${head}:${policy}`;
  ...
  await assert.rejects(() => runEarlyFence(async (route, parameters) => {
    ...
    throw new Error(`unexpected request ${route}`);
  }), /unexpected owner or identity/);
  assert.deepEqual(calls.filter(({ route }) => route.startsWith('PATCH ') || route.startsWith('POST ')), []);
});

test('early fence replaces a completed current generation without rewriting completed history', async () => {
  const head = 'a'.repeat(40);
  const base = 'b'.repeat(40);
  const policy = 'c'.repeat(40);
  const external = `aeris-policy:v1:123:37:${head}:${policy}`;
  const oldExternal = 'aeris-policy:v1:123:37:older';
  const state = new Map([
    [88, { id: 88, name: 'Automation Policy / gate', head_sha: head, external_id: oldExternal, status: 'completed', conclusion: 'success', app: { id: 9001, slug: 'aeris-token-policy' } }],
    [77, { id: 77, name: 'Automation Policy / gate', head_sha: head, external_id: external, status: 'completed', conclusion: 'success', app: { id: 9001, slug: 'aeris-token-policy' } }],
  ]);
  const mutations = [];
  const outputs = await runEarlyFence(async (route, parameters) => {
    if (route === 'GET /repos/{owner}/{repo}') return { data: { id: 123, full_name: 'JinPengGeng/aeris-token', default_branch: 'main' } };
    ...
    if (route === 'POST /repos/{owner}/{repo}/check-runs') {
      ...
      const next = { id: 99, conclusion: null, app: { id: 9001, slug: 'aeris-token-policy' }, ...parameters };
      state.set(99, next);
      return { data: next };
    }
    if (route === 'GET /repos/{owner}/{repo}/check-runs/{check_run_id}') return { data: state.get(parameters.check_run_id) };
  ...
  assert.deepEqual(outputs, { head_sha: head, policy_sha: policy, check_run_id: '99' });
  assert.equal(mutations.length, 1);
  assert.equal(mutations[0].external_id, external);
  assert.equal(state.get(88).external_id, oldExternal);
