...
import { executorForRoute, validateExecutorRegistry } from '../src/ai-executor-contract.mjs';
...
test('registry is a static exact allowlist and production routes remain chat', () => {
  ...
  assert.deepEqual(executorForRoute(normalized, 'agent_analysis'), registry.executors[0]);
...
test('factory returns a trusted chat adapter and does not select from environment', async (t) => {
  ...
  assert.deepEqual(result.executor, registry.executors[0]);
