...
test('dispatch targets fail before Check Run mutation when the PR identity is invalid', async (t) => {
  const cases = [
    ...
    ['missing base repository name', pull({ base: { ref: 'main', repo: {} } }), /pull request base repository name is invalid/],
    ['missing base ref', pull({ base: { repo: { full_name: CONFIG.repository } } }), /pull request base ref is invalid/],
    ['missing head projection', pull({ head: null }), /pull request head is invalid/],
    ['missing head repository projection', pull({ head: { ref: 'agent/issue-7', sha: SHA } }), /pull request head repository is invalid/],
    ['missing head repository name', pull({ head: { ref: 'agent/issue-7', sha: SHA, repo: {} } }), /pull request head repository name is invalid/],
    ['missing head ref', pull({ head: { sha: SHA, repo: { full_name: CONFIG.repository } } }), /pull request head ref is invalid/],
    ['missing head SHA', pull({ head: { ref: 'agent/issue-7', repo: { full_name: CONFIG.repository } } }), /pull request head SHA is invalid/],
    ['missing user projection', pull({ user: null }), /pull request user is invalid/],
    ['missing user type', pull({ user: { login: CONFIG.writer_login } }), /pull request user type is invalid/],
    ['missing user login', pull({ user: { type: 'Bot' } }), /pull request user login is invalid/],
