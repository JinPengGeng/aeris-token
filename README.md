...
- 生产 Redis 持久化：在 `docker-compose.yml` 或 `docker-compose.single-node.yml` 之后叠加 `docker-compose.redis-durable.yml`。该 profile 使用 `/data` 命名卷、AOF `everysec` 和恢复演练；基础/local Compose 保持轻量的非持久默认，详见 [Redis 运行手册](docs/operations/redis-runtime-runbook.md)
...
- `AETHER_GATEWAY_USAGE_QUEUE_DLQ_MAXLEN`：死信 stream 的最大保留条数，默认 `50000`。写入和内置 Redis/Memory 原子转移都使用近似 `MAXLEN`，超过上限优先淘汰最旧条目，避免 poison message 无限占用 Redis；设为 `0` 无效并在启动配置校验时拒绝。该上限是容量保护，不替代审计归档或 redrive，生产环境应配合 `usage_queue_dlq_length` 告警和独立留存策略
