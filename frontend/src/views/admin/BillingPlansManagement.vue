              ...
              <template v-if="form.carry_over">
                <div class="space-y-1.5">
                  ...
                  <Select v-model="carryOverDaysSelectValue">
                    ...
                    <SelectContent>
                      <SelectItem v-for="option in carryOverDayOptions" :key="option.value" :value="option.value">{{ option.label }}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div class="space-y-1.5">
                  ...
                  <Select v-model="carryOverLimitMultiplierSelectValue">
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem v-for="option in carryOverLimitMultiplierOptions" :key="option.value" :value="option.value">{{ option.label }}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </template>
...
const carryOverLimitMultiplierOptions = computed(() => {
  ...
  return uniqueNumericOptions([1, 1.25, 1.5, 2, 3, 5, 10, maxMultiplier, currentMultiplier])
    .filter((value) => value <= maxMultiplier)
    .map((value) => ({ value: formatCarryOverNumberValue(value), label: formatCarryOverMultiplierLabel(value) }))
})

const carryOverLimitMultiplierSelectValue = computed({
  get: () => {
    const maxMultiplier = Math.min(31, Number(form.carry_over_days || 1) + 1)
    return formatCarryOverNumberValue(Math.min(maxMultiplier, Math.max(1, Number(form.carry_over_limit_multiplier) || 1)))
  },
  set: (value: string) => {
...
function buildEntitlements(): BillingEntitlement[] {
  ...
  if (form.daily_quota_enabled) {
    ...
    if (form.carry_over) {
      entitlement.carry_over = true
      entitlement.carry_over_days = Math.floor(Number(form.carry_over_days))
      entitlement.carry_over_limit_multiplier = Number(form.carry_over_limit_multiplier)
    }
