...
async fn resolve_wallet_auth_gate_with_cache(
    ...
    {
        return Ok(Some(
            resolve_package_billing_source_decision(state, &auth_snapshot.user_id, use_cache)
                .await?,
        ));
    }
...
async fn resolve_package_billing_source_decision(
    ...
    let quota = if use_cache {
        state
            .find_user_daily_quota_availability_for_auth(user_id)
            .await?
    } else {
        state
    ...
    let quota = quota.filter(|quota| quota.has_active_daily_quota);
