...
#[test]
fn classifies_admin_usage_margin_stats_as_admin_proxy_route() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/usage/margin/stats"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::GET, &uri, &headers).expect("route should classify");
    ...
    assert_eq!(decision.route_class.as_deref(), Some("admin_proxy"));
    assert_eq!(decision.route_family.as_deref(), Some("usage_manage"));
    assert_eq!(decision.route_kind.as_deref(), Some("margin_report"));
    assert_eq!(
        decision.auth_endpoint_signature.as_deref(),
        Some("admin:usage")
    );
...
fn classifies_admin_usage_dlq_list_as_delegated_read() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/usage/dlq?limit=10"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::GET, &uri, &headers).expect("route should classify");
