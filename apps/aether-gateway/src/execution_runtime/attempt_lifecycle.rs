...
/// One physical upstream dispatch's replay-safety state.
///
/// This state records observable execution facts only. Replay eligibility is
/// additionally constrained by [`ReplayBarrier`], which is deliberately kept
/// separate so a conservative request classification never fabricates a
/// client commit.
...
/// Replay safety is independent from the physical-dispatch state.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// The retry policy decision evaluated at the point a replay permit is issued.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum ReplayPolicyDecision {
    Allow,
    Deny,
}
...
pub(crate) enum AttemptDispatchLifecycleError {
    ...
    #[error("retry policy denied replay")]
    ReplayPolicyDenied,
    #[error("a replay permit was already issued for this attempt")]
    ReplayPermitAlreadyIssued,
    #[error("the replay permit was already consumed")]
    ReplayPermitAlreadyConsumed,
}

/// An opaque, non-cloneable authorization to create exactly one new physical
/// dispatch lifecycle.
///
/// A permit is returned only after the previous lifecycle has atomically moved
/// to `Terminal`. `start_next_attempt` consumes its capability at runtime as
/// well as conceptually, which makes accidental duplicate consumption visible
/// in tests and in adapter code.
...
impl ReplayPermit {
    pub(crate) fn start_next_attempt(
        &mut self,
        ...
        self.consumed = true;
        Ok(AttemptDispatchLifecycle::new())
    }
}

/// Transport-neutral replay-safety lifecycle for one physical dispatch.
///
/// This core is intentionally in-process and scoped to one logical request.
/// Durable idempotency and cross-process recovery require a separate ledger.
#[derive(Debug)]
pub(crate) struct AttemptDispatchLifecycle {
    state: AttemptDispatchState,
    replay_barrier: ReplayBarrier,
    replay_permit_issued: bool,
}

impl Default for AttemptDispatchLifecycle {
    fn default() -> Self {
        Self::new()
    }
}
...
impl AttemptDispatchLifecycle {
    ...
    pub(crate) const fn replay_barrier(&self) -> ReplayBarrier {
        self.replay_barrier
    }

    pub(crate) const fn replay_permit_issued(&self) -> bool {
        self.replay_permit_issued
    }

    /// Record the single physical dispatch owned by this lifecycle.
    pub(crate) fn mark_sent(&mut self) -> Result<(), AttemptDispatchLifecycleError> {
        self.transition_to(AttemptDispatchState::SentButUncommitted)
    }

    /// Record the gateway's irreversible application-layer response handoff.
    ///
    /// This does not claim that the client actually received the response.
    ...
    /// Close the replay barrier without changing execution facts.
    ///
    /// The first close reason wins. Repeated conservative observations are
    /// idempotent, but no mutation is accepted after terminal settlement.
    pub(crate) fn close_replay_barrier(
        &mut self,
        reason: ReplayBarrierReason,
    ) -> Result<bool, AttemptDispatchLifecycleError> {
        if self.state == AttemptDispatchState::Terminal {
        ...
        if !self.replay_barrier.is_open() {
            return Ok(false);
        }
        self.replay_barrier = ReplayBarrier::Closed(reason);
        Ok(true)
    }

    /// Settle without authorizing another physical dispatch.
    pub(crate) fn settle(&mut self) -> Result<(), AttemptDispatchLifecycleError> {
        self.transition_to(AttemptDispatchState::Terminal)
    }

    /// Issue one replay permit and settle the current physical lifecycle.
    ///
    /// Eligibility is evaluated while this lifecycle is still uncommitted and
    /// non-terminal. The permit is created before the state moves to Terminal,
    /// but is returned only after that move succeeds, preventing overlapping
    /// old/new physical attempts through this API.
    pub(crate) fn settle_for_replay(
        &mut self,
        policy: ReplayPolicyDecision,
        ...
        match self.state {
            ...
            AttemptDispatchState::Prepared | AttemptDispatchState::SentButUncommitted => {}
        }
        if let ReplayBarrier::Closed(reason) = self.replay_barrier {
            return Err(AttemptDispatchLifecycleError::ReplayBarrierClosed(reason));
        }
        if policy == ReplayPolicyDecision::Deny {
            return Err(AttemptDispatchLifecycleError::ReplayPolicyDenied);
        }
        if self.replay_permit_issued {
            return Err(AttemptDispatchLifecycleError::ReplayPermitAlreadyIssued);
        }

        self.replay_permit_issued = true;
        let permit = ReplayPermit { consumed: false };
        self.transition_to(AttemptDispatchState::Terminal)?;
        Ok(permit)
    }
...
#[cfg(test)]
mod dispatch_lifecycle_tests {
    use super::{
        AttemptDispatchLifecycle, AttemptDispatchLifecycleError, AttemptDispatchState,
        ReplayBarrier, ReplayBarrierReason, ReplayPolicyDecision,
    };

    fn lifecycle_in(state: AttemptDispatchState) -> AttemptDispatchLifecycle {
        ...
        for from in [Prepared, SentButUncommitted, ClientCommitted, Terminal] {
            for to in [Prepared, SentButUncommitted, ClientCommitted, Terminal] {
                ...
                assert_eq!(
                    ...
                    "an invalid transition must not mutate state"
                );
            }
        }
    }
    ...
    #[test]
    fn one_lifecycle_cannot_dispatch_twice_or_settle_twice() {
        let mut lifecycle = AttemptDispatchLifecycle::new();
        lifecycle.mark_sent().expect("first dispatch is valid");
        assert_eq!(
            lifecycle.mark_sent(),
            Err(AttemptDispatchLifecycleError::InvalidTransition {
                from: AttemptDispatchState::SentButUncommitted,
                to: AttemptDispatchState::SentButUncommitted,
            })
        );

        lifecycle.settle().expect("first settlement is valid");
        assert_eq!(
            lifecycle.settle(),
            Err(AttemptDispatchLifecycleError::InvalidTransition {
                from: AttemptDispatchState::Terminal,
    ...
    #[test]
    fn replay_barrier_closes_monotonically_without_fabricating_commit() {
        let mut lifecycle = AttemptDispatchLifecycle::new();
        lifecycle.mark_sent().expect("dispatch is valid");

        assert!(lifecycle
            .close_replay_barrier(ReplayBarrierReason::ToolCall)
            .expect("the first close succeeds"));
        assert_eq!(
            lifecycle.state(),
            AttemptDispatchState::SentButUncommitted,
            "closing the barrier must not claim that the client was committed"
        );
        assert_eq!(
            lifecycle.replay_barrier(),
            ReplayBarrier::Closed(ReplayBarrierReason::ToolCall)
        );
        assert!(!lifecycle
            .close_replay_barrier(ReplayBarrierReason::CompactOperation)
            .expect("a repeated close is idempotent"));
        assert_eq!(
            lifecycle.replay_barrier(),
            ReplayBarrier::Closed(ReplayBarrierReason::ToolCall),
            "the first conservative close reason remains authoritative"
        );

        assert_eq!(
            lifecycle.settle_for_replay(ReplayPolicyDecision::Allow),
            Err(AttemptDispatchLifecycleError::ReplayBarrierClosed(
    ...
    #[test]
    fn replay_requires_policy_and_an_uncommitted_lifecycle() {
        let mut denied = AttemptDispatchLifecycle::new();
        denied.mark_sent().expect("dispatch is valid");
        assert_eq!(
            denied.settle_for_replay(ReplayPolicyDecision::Deny),
            Err(AttemptDispatchLifecycleError::ReplayPolicyDenied)
        );
        assert_eq!(denied.state(), AttemptDispatchState::SentButUncommitted);
        assert!(!denied.replay_permit_issued());

        let mut committed = AttemptDispatchLifecycle::new();
        committed.mark_sent().expect("dispatch is valid");
        committed
            .mark_client_committed()
            .expect("the irreversible handoff follows dispatch");
        assert_eq!(
            committed.settle_for_replay(ReplayPolicyDecision::Allow),
            Err(AttemptDispatchLifecycleError::ClientCommitted)
        );
        assert_eq!(committed.state(), AttemptDispatchState::ClientCommitted);
    }

    #[test]
    fn replay_permit_is_issued_before_terminal_and_consumed_once() {
        let mut lifecycle = AttemptDispatchLifecycle::new();
        lifecycle.mark_sent().expect("dispatch is valid");
        let mut permit = lifecycle
            .settle_for_replay(ReplayPolicyDecision::Allow)
            .expect("an uncommitted open attempt may be replayed");

        assert!(lifecycle.replay_permit_issued());
        assert_eq!(lifecycle.state(), AttemptDispatchState::Terminal);
        assert!(matches!(
            lifecycle.settle_for_replay(ReplayPolicyDecision::Allow),
            Err(AttemptDispatchLifecycleError::InvalidTransition {
                from: AttemptDispatchState::Terminal,
        ...
        let next = permit
            .start_next_attempt()
            .expect("the permit creates one fresh physical lifecycle");
        assert_eq!(next.state(), AttemptDispatchState::Prepared);
        assert_eq!(next.replay_barrier(), ReplayBarrier::Open);
        assert_eq!(
    ...
    #[test]
    fn prepared_and_committed_attempts_can_reach_terminal_without_replay() {
        let mut never_sent = AttemptDispatchLifecycle::new();
        never_sent.settle().expect("prepared may terminate");
        assert_eq!(never_sent.state(), AttemptDispatchState::Terminal);

        let mut committed = AttemptDispatchLifecycle::new();
        committed.mark_sent().expect("dispatch is valid");
        committed
            .mark_client_committed()
            .expect("commit is valid after dispatch");
        committed.settle().expect("committed may terminate");
        assert_eq!(committed.state(), AttemptDispatchState::Terminal);
    }

    #[test]
    fn all_conservative_request_kinds_close_the_same_monotonic_barrier() {
        for reason in [
            ...
            lifecycle.mark_sent().expect("dispatch is valid");
            lifecycle
                .close_replay_barrier(reason)
                .expect("barrier close is valid before terminal");
            assert_eq!(lifecycle.replay_barrier(), ReplayBarrier::Closed(reason));
            assert!(matches!(
                lifecycle.settle_for_replay(ReplayPolicyDecision::Allow),
                Err(AttemptDispatchLifecycleError::ReplayBarrierClosed(actual))
                    if actual == reason
            ));
        }
    }
