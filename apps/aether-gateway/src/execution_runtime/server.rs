...
mod tests {
    ...
    #[tokio::test]
    async fn execution_runtime_first_byte_timeout_is_a_sanitized_bad_gateway() {
        let response = ExecutionRuntimeAppError(ExecutionRuntimeServerError::Transport(
            ExecutionRuntimeTransportError::UpstreamFirstByteTimeout(
                "Bearer upstream-secret https://user:password@example.test".to_string(),
            ),
        ))
        .into_response();
