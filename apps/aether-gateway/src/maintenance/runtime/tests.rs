...
use super::{
    ...
    spawn_provider_checkin_worker, spawn_provider_credential_sweep_worker,
...
#[tokio::test]
async fn spawn_provider_credential_sweep_worker_skips_when_provider_catalog_unavailable() {
    let state = AppState::new()
        .expect("gateway state should build")
        .with_data_state_for_tests(GatewayDataState::disabled());
    assert!(spawn_provider_credential_sweep_worker(state).is_none());
}
