...
use super::{
    ...
    perform_oauth_token_refresh_once, perform_provider_credential_sweep_once,
    perform_provider_quota_alert_once, perform_remote_quota_sync_once, provider_checkin_schedule,
    remote_quota_sync_worker_interval, run_audit_cleanup_once, run_data_lifecycle_cleanup_once,
    ...
    POOL_MONITOR_INTERVAL, PROVIDER_CHECKIN_DEFAULT_TIME, PROVIDER_CREDENTIAL_SWEEP_INTERVAL,
...
pub(crate) fn spawn_provider_credential_sweep_worker(
    state: AppState,
) -> Option<tokio::task::JoinHandle<()>> {
    if !state.has_provider_catalog_data_reader() || !state.has_provider_catalog_data_writer() {
        return None;
    ...
    Some(crate::task_runtime::spawn_singleton_worker(
        state,
        crate::task_runtime::TASK_KEY_PROVIDER_CREDENTIAL_SWEEP,
        |state| async move {
            if let Err(err) = perform_provider_credential_sweep_once(&state).await {
                log_maintenance_worker_failure("provider_credential_sweep", "startup", &err);
            }
            let mut interval = tokio::time::interval(PROVIDER_CREDENTIAL_SWEEP_INTERVAL);
            interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
            interval.tick().await;
            loop {
                interval.tick().await;
                if let Err(err) = perform_provider_credential_sweep_once(&state).await {
                    log_maintenance_worker_failure("provider_credential_sweep", "tick", &err);
                }
            }
        },
    ))
