...
- The special-value boundary fixture confirms that `Infinity`, `-Infinity`,
  and `NaN` are representable only by unconstrained PostgreSQL `numeric` in
  this context and are rejected by the application's `numeric(20,8)` typmod;
  it performs no application-table writes.
