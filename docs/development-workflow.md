...
同步不会绕过 PR 直接写入 `main`。`.github/upstream-sync-policy.yml` 中的 fork-owned 路径在三方合并前被过滤并保留 fork 版本；auth/migrations/security 与未知路径发布为人工审查 PR，`.gitmodules`、`.pem`、`.key`、`.p12` 等敏感文件 fail closed。`Automation Policy / gate` 对同步只证明 required-check health，不是 eligibility attestation；只有 trusted prepare 输出为 `eligible`、精确 commit trailer 和 merge helper 再次校验全部通过时才允许 direct merge。干净生成的 managed 同步 PR 只会在受保护分支要求的检查于精确 head/base SHA 上成功的有界等待期内，使用一次 `PUT /repos/{owner}/{repo}/pulls/{number}/merge`、`merge_method=squash` 和该 SHA 尝试合并；不会启用 GitHub native auto-merge。无论 mutation 的响应成功、失败或不确定，工作流都只独立回读一次，且必须证明同一 PR 由 Writer App bot 合并、head/base 一致、`auto_merge=null`，以及 merge commit 是以 base 为唯一 parent 的 squash commit，才记为成功。检查未成功或超时、mutation/readback 失败、冲突、上游历史重写、非法 state/policy 和无法识别的同步分支 tip 均 fail closed，不会推进 checkpoint 或覆盖远端分支；固定分支和开放 PR 会被保留，供后续同步复用。重建固定分支前仍会 disarm 历史遗留的 native auto-merge，避免旧授权残留。人工解决真实冲突时，应使用普通维护者 PR 同时提交解决结果和新的 `last_integrated_sha`。
