...
expiry header，剩余有效期不得超过 24 小时；缺少该 header 仅能记录为 operator evidence，不能当作 API 证明。
...
通过后才写入上述两个身份变量并关闭 bootstrap flag；同一 workflow run 随后按顺序调用固定 caller revision 的 local
reusable read-only attestation 与 governance canary，而不是以可变 `main` ref REST dispatch。called jobs 不继承
bootstrap control token。该 workflow 不会开启任何生产开关；成功后必须立即删除临时 Environment secret 并撤销该
短期 token。
...
验证且字节级归一化结果一致，否则 fail closed。step summary 仅保存非敏感的 snapshot SHA-256、精确 ruleset
ID 和闭合字段的 snapshot 摘要；不得输出或保存 private key、installation token 或原始 API 响应。
