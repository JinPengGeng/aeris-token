...
| fork 发布身份与 aeris-token-v tag 命名空间 | 本 PR | `install.sh` 默认 REPO/IMAGE_REPO 改为 JinPengGeng/aeris-token 与 ghcr.io/jinpenggeng/aeris-token、channel 解析接受 `aeris-token-v` 前缀；`docker-compose*.yml`/`.env.example` 默认镜像改为本仓 GHCR；`release.yml`/`deploy-pages.yml` 只响应 `aeris-token-v*` tag、镜像 tag 去前缀。这些是 fork 身份的**永久分歧点**，不回销上游 | 永久 fork 定制；sync 时这些文件按 fork-owned 语义逐点重落位，不得被上游默认值覆盖 |
| Sub2API 远程配额感知（#174，PR-A #277 后端核心 + PR-B 管理面） | #277 + 本 PR | 上游 fawney19/Aether#727 语义的 fork 精简重设计：`provider_ops.remote_quota` 配置与响应解析/校验（`crates/aether-admin/src/provider/ops/remote_quota.rs`）、Sub2API 拉取 facade（`apps/aether-gateway/src/handlers/admin/provider/ops/providers/remote_quota.rs`）、`remote_quota_sync` 周期 worker（`apps/aether-gateway/src/maintenance/runtime/remote_quota_sync.rs`，及 `workers.rs`/`task_runtime/mod.rs`/`state/core.rs` 注册点）；耗尽/失效映射为既有 key 熔断（`remote_quota_managed` 标记，只管理自己写的熔断），数值明细写 `status_snapshot.quota`；默认关闭。PR-B：`sync_remote_quota` provider-ops action（`actions/sync_remote_quota.rs` + 路由 4xx 语义）、provider summary `ops_remote_quota` 只读透出（`summary/value.rs`，白名单投影在 aether-admin）、ops config 保存保留 `remote_quota` 键（`ops/providers/config.rs`）、前端 `ProviderRemoteQuotaCard` + `useProviderRemoteQuota` + providerOps API | 不移植上游 #727 存储层与新调度约束层（复用既有熔断 + status_snapshot）；sync 时上述文件按 fork-owned 语义逐点重落位，不得与上游 #727 的配额快照仓储/调度过滤代码混合 |
| 日期戳 nightly 发布（`aeris-token-nightly-YYYYMMDD`，方案 B） | 本 PR | `.github/workflows/nightly.yml` fork 自有：每日 UTC 日期戳 tag（immutable releases 下 tag 永不复用）、main 顶端三项 required checks 质量门、GHCR 推送滚动 `:nightly` + 日期戳 `:nightly-YYYYMMDD` 双 tag、`gh release create` draft→传资产→公布的 immutable 兼容流程、保留最近 7 个 nightly Release（Release 删除后删 tag，只匹配 `aeris-token-nightly-` 前缀）；`install.sh --channel nightly` 改为解析最新日期戳 nightly tag，compose 镜像 tag 保持 `:nightly` 滚动、与 Release tag 解耦 | 永久 fork 定制；上游 nightly.yml 在本仓保持不存在、不回销——上游滚动同名 tag 模式与本仓 immutable releases 不兼容；sync 时 `.github/**` 按 fork-owned 语义处理 |
