...
| permit 在 response relay 前释放 | 设计取舍，P2 | `stream_handler.rs:2071-2072,2123-2124,2190` 明确在 relay 前 drop；连接/stream 另有独立上限，避免长流占住请求准入闸。 | stream handler admission 测试。 | 明确两类并发预算职责，不把该行为标为已修。 |
