...
Writer ref 不通过缺少 expected-old-SHA 条件的 REST PATCH 推进。publish 使用 App installation token 执行 `git push --force-with-lease=<exact-ref>:<expected-old-sha>`；创建 ref 时 expected old 为空，因此 ref 在并发窗口中被创建、推进或替换都会由 Git 接收端原子拒绝。每个底层 mutation callback 完成命令、Issue、权限等网络读取后，仍把 exact main/ref/完整 App-owned Draft PR ownership/lifecycle 快照作为最后一次写前 fence，并在写后复读 main、ref、PR 和签名 receipt marker。现有 Draft PR title/body 没有 GitHub CAS，因此 Writer 不发送 metadata PATCH，只允许目标 metadata 已逐字节一致时的 read-only no-op；人工或并发漂移在 ref push 前 fail closed。v2 receipt marker 绑定 issue、触发 comment、受信 lifecycle epoch、跨运行 fix cycle、candidate SHA 与 commit SHA，并由 Writer App 私钥生成域隔离 RSA-SHA256 签名；v1 receipt 或非当前 epoch receipt 均不可信。由于 retry 的新 receipt 必然改变 PR body，当前 `/agent retry-write` 会在任何 mutation 前以 `managed_pr_metadata_change_requires_cas` 停止，直到存在可原子更新的权威 receipt 存储。签名 receipt 不是 lifecycle 权威来源：完整 PR timeline 中只要出现历史 `closed` 事件，Issue 就在 epoch 0 中永久 tombstone；即使 GitHub 当前返回 open/reopened 且旧 receipt 仍有效，也不得再次写入。timeline 权限/API 失败、畸形事件或三页截断都必须 fail closed。POST 建 PR 的不确定结果则始终有界枚举同一 head 的所有 open/closed PR，要求本次 attempt marker 全局唯一，并在返回前再次读取、验证 App 所有者、draft/open、base/head SHA、完整 marker/body 和分支 head。这个只读对账流程缩窄竞态窗口并使未确认结果可见，但不会关闭 PR、删除分支或执行 metadata overwrite；无法证明结果时报告残留而不继续写入。
