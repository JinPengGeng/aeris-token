...
VSCODEX_DOCKERFILE="${REPO_ROOT}/aether-vscodex/Dockerfile"
...
if grep -Eq '^FROM[[:space:]]+[^[:space:]@]+(:[^[:space:]@]+)?([[:space:]]+AS[[:space:]]+[^[:space:]]+)?$' "${VSCODEX_DOCKERFILE}"; then
    fail_test "VSCodex Dockerfile contains an unpinned base image"
fi
